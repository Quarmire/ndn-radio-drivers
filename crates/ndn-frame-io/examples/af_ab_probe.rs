...
impl Sock {
    fn open(iface: &str) -> Self {
        ...
        a.sll_protocol = ETH_P_ALL.to_be();
    ...
    fn raw_send(&self, buf: &[u8]) -> isize {
        ...
        d.sll_family = libc::AF_PACKET as u16;
        d.sll_protocol = ETH_P_ALL.to_be();
        d.sll_ifindex = self.ifindex;
        unsafe {
            libc::sendto(
                ...
                buf.len(),
                0,
                &d as *const _ as *const libc::sockaddr,
                std::mem::size_of::<libc::sockaddr_ll>() as u32,
            )
        }
    }
    ...
    async fn inject_new(&self, buf: &[u8]) -> std::io::Result<()> {
        ...
        loop {
            ...
            let _ = tokio::time::timeout(Duration::from_millis(20), async {
                if let Ok(mut g) = self.fd.writable().await {
                    g.clear_ready();
                }
            })
            .await;
...
async fn main() {
    ...
    let buf: Arc<Vec<u8>> = Arc::new({
        ...
        let mut v = vec![0u8, 0, 8, 0, 0, 0, 0, 0];
        v.extend_from_slice(&[0x5a; 900]);
        v
    });
    println!(
        "mode={mode} iface={iface}: RX drainer + 400 CONCURRENT injects (matches the live face)…"
    );
    ...
    for i in 0..400u32 {
        ...
        handles.push(tokio::spawn(async move {
            ...
            let r = tokio::time::timeout(Duration::from_secs(8), async move {
                if m == "old" {
                    s.inject_old(&b).await
                } else {
                    s.inject_new(&b).await
                }
            })
            .await;
            match r {
    ...
    if hung > 0 {
        println!("=> {hung} injects never returned within 8s: THIS logic wedges.");
    } else {
        println!("=> every inject returned (no permanent hang): THIS logic is robust.");
    }
}
