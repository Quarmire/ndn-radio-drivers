...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if lbt {
        ...
        let peak: u8 = env("NDN_CAD_PEAK")
            .and_then(|s| s.parse().ok())
            .unwrap_or(18);
        let min: u8 = env("NDN_CAD_MIN")
            .and_then(|s| s.parse().ok())
            .unwrap_or(10);
        if let Err(e) = dev.set_cad_cfg(sym, peak, min) {
            eprintln!("set_cad_cfg: {e:?}");
        }
    }
    dev.set_lbt(lbt);
    ...
    println!(
        "lora_contention tag={tag} path={path} ch={ch}(={}MHz) secs={secs} tx_ms={tx_ms} LBT={}",
        850 + ch as u32,
        if lbt { "ON" } else { "off" }
    );
    ...
    println!(
        "=== tag={tag} LBT={} === sent={sent} txfail={txfail} heard_from_peers={} (n0={c0} n1={c1} n2={c2}) cad_busy={cad_busy} deferred={deferred}",
        if lbt { "ON" } else { "off" },
        c0 + c1 + c2
    );
    use std::io::Write;
