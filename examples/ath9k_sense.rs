...
fn main() -> ExitCode {
    ...
    for i in 0..6 {
        let tsf = RadioTime::read_clock(&dev, domain).ok().flatten().unwrap_or(0);
        let occ = RadioKnobs::read_channel_activity(&dev).ok().flatten().unwrap_or(0);
        let dtsf = if i == 0 { 0 } else { tsf.wrapping_sub(last_tsf) };
        let docc = if i == 0 { 0 } else { occ.wrapping_sub(last_occ) };
