...
const PWR_TX_RATE: [u32; 9] = [
    0x9934, 0x9938, 0xa234, 0xa238, 0xa38c, 0xa390, 0xa3cc, 0xa3d0, 0xa3d4,
];
...
fn main() -> ExitCode {
    ...
    let orig: Vec<u32> = PWR_TX_RATE
        .iter()
        .map(|&a| dev.reg_read(a).unwrap_or(0))
        .collect();
    ...
    async fn burst(dev: &Ath9kHtcBackend, tag: u8, n: usize) {
        for i in 0..n {
            let f = InjectFrame::broadcast(
                Bytes::copy_from_slice(&[0x05, 0x08, tag, i as u8]),
    ...
    let _ = dev.detach();
    println!(
        "done — compare witness RSSI across the three ~4s windows (W1 t0-4, W2 t8-12, W3 t16-20)."
    );
    ExitCode::SUCCESS
}
