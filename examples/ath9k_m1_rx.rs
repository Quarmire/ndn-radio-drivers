...
fn run(dev: &mut Ath9kHtcBackend, channels: &[u16]) -> ExitCode {
    ...
    println!(
        ...
        iv.matched,
        iv.checked,
        if iv.mismatches.is_empty() {
            ""
        } else {
            " (analog 0x7804 hw-bits expected)"
        }
    );
    ...
    if !cal.agc_cal_converged {
        eprintln!(
            "[M1.3] AGC cal hung — AR_PHY_AGC_CONTROL={:#010x} (CAL bit stuck). RX may be deaf.",
            cal.agc_control
        );
    ...
    if cal.phy_active & 0x1 != 0x1 {
        eprintln!(
            "[M1.3] WARNING: AR_PHY_ACTIVE did not read back enabled ({:#x})",
            cal.phy_active
        );
    }
    ...
    if std::env::var_os("NDR_FORCE_KERNEL").is_some() {
        ...
        for &(a, v) in KERNEL_REGS {
            if a == 0x000c {
                continue;
            } // RXDP owned by the target
            if dev.reg_write(a, v).is_ok() {
                n += 1;
            }
        }
