...
async fn main(_spawner: Spawner) {
    ...
    let (mut radio, timing) = hw::init(p);
    ...
    let (mut got, mut bad) = (0u32, 0u32);
    let mut latency = Spread::default();   // CC[1] - CC[0]: the software path we are replacing
    let mut interarrival = Spread::default(); // hw stamp deltas: an UPPER BOUND (see module docs)
    ...
    loop {
        ...
        if let Some(prev) = prev_hw {
            interarrival.push(hw.since(prev));
        }
        prev_hw = Some(hw);
        ...
        if got % REPORT_EVERY == 0 {
            ...
            defmt::info!(
                "        | hw inter-arrival (UPPER BOUND, incl. TX jitter) min={=u32} mean={=u32} max={=u32} p2p={=u32} ticks | crc_err {}",
                interarrival.min,
                interarrival.mean(),
                interarrival.max,
