...
//! them as configuration instead of transmitting. That is the only way to reach the lease/quiet
//! config while `ath9k_htc` owns the device — the WMI path needs the device, and the PHY needs the
//! kernel driver.
//!
//! ```sh
...
fn main() {
    ...
    if a.len() < 3 {
        eprintln!(
            "usage: {} <iface> <lease|quiet-off> [args]",
            a[0]
        );
        std::process::exit(1);
    }
    ...
    let (op, payload): (u8, Vec<u8>) = match a[2].as_str() {
        // The in-frame name filter is retired (see firmware/NDR_MAC_SPEC.md); only the
        // lease/quiet control ops remain.
        "lease" => (
            0x07,
            vec![
