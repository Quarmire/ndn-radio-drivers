...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while start.elapsed().as_secs() < secs {
        ...
        println!(
            "{:>6.1} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>9}",
            start.elapsed().as_secs_f64(),
            d(c.ofdm_ok, prev.ofdm_ok),
            d(c.ofdm_err, prev.ofdm_err),
            d(c.ofdm_fa, prev.ofdm_fa),
            d(c.cck_ok, prev.cck_ok),
            d(c.cck_err, prev.cck_err),
            d(c.cck_fa, prev.cck_fa),
            d(c.ht_ok, prev.ht_ok),
            d(c.ht_err, prev.ht_err),
            d(c.ht_fa, prev.ht_fa),
            d(cca, prev_cca)
        );
        prev = c;
        prev_cca = cca;
