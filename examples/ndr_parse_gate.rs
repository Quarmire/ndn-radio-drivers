...
fn parse_name(pkt: &[u8]) -> Option<String> {
    ...
    let mut i = 0;
    let _t = rd_var(pkt, &mut i)?;
    let ln = rd_var(pkt, &mut i)?;
    let val = pkt.get(i..i + ln)?;
    let mut j = 0;
    while j < val.len() {
        let tt = rd_var(val, &mut j)?;
        let ll = rd_var(val, &mut j)?;
        let sub = val.get(j..j + ll)?;
        if tt == 0x07 {
            let mut k = 0;
            let mut s = String::new();
            while k < sub.len() {
                let _ct = rd_var(sub, &mut k)?;
                let cl = rd_var(sub, &mut k)?;
                s.push('/');
                s.push_str(&String::from_utf8_lossy(sub.get(k..k + cl)?));
                k += cl;
            }
            return Some(s);
        }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let rx_port = a
        .get(1)
        .cloned()
        .unwrap_or_else(|| "/dev/cu.usbmodem101".into());
    let tx_port = a
        .get(2)
        .cloned()
        .unwrap_or_else(|| "/dev/cu.usbmodem11401".into());
