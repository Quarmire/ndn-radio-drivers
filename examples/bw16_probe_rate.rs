...
async fn run() -> Result<(), Box<dyn std::error::Error>> {
    use bytes::Bytes;
    use ndn_frame_io::{FrameFormat, FrameIo, InjectFrame, TxIntent, frame};
    ...
    println!(
        "probing pkt_attrib offsets 0..{max_off} with rate=0x{rate_code:02x}; 8812EU MCS per offset:"
    );
    ...
    for off in 0..=max_off {
        ...
        while Instant::now() < deadline {
            if let Ok(Ok(fr)) =
                tokio::time::timeout(Duration::from_millis(150), eu.recv_frame()).await
            {
        ...
        if ht || mcs.values().sum::<u32>() == 0 {
            println!(
                "  off {off:2} (0x{off:02x}): {mcs:?}{}",
                if ht { "  <== HT!" } else { "  (no capture)" }
            );
            if ht {
                hits.push(off);
            }
