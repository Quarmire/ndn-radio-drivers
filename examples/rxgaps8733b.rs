...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut gaps: Vec<u64> = stamps.windows(2).map(|w| w[1].wrapping_sub(w[0])).filter(|&g| g < 5_000_000).collect();
    gaps.sort_unstable();
    let pct = |p: f64| gaps[((gaps.len() - 1) as f64 * p) as usize];
    println!("inter-arrival gaps (us): p50={} p90={} p99={} max={}", pct(0.5), pct(0.9), pct(0.99), gaps[gaps.len() - 1]);
    ...
    let names = ["<100us", "<500us", "<1ms", "<5ms", "<10ms", "<20ms", "<50ms", ">=50ms"];
    let mut counts = [0usize; 8];
    for g in &gaps {
        for (i, b) in buckets.iter().enumerate() {
            if g < b { counts[i] += 1; break; }
        }
    }
    println!("gap histogram:");
    for (n, c) in names.iter().zip(counts) {
        if c > 0 {
            println!("  {n:>8} {c:>6}  {:.1}%", 100.0 * c as f64 / gaps.len() as f64);
        }
    }
    let big: usize = counts[3..].iter().sum();
    println!("\ngaps >= 1ms: {big} ({:.1}%) — a shaped duty cycle puts real mass here; \
              pure deferral does not.", 100.0 * big as f64 / gaps.len() as f64);
    Ok(())
}
