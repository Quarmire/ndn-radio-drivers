...
fn main() -> ExitCode {
    ...
    let show = |dev: &Ath9kHtcBackend, tag: &str| {
        match dev.ndr_stats() {
            Ok(s) => println!(
                "  [{tag:>9}] seen={} passed={} dropped_filter={} dropped_foreign={} short={} popcount={}",
                s.seen, s.passed, s.dropped_filter, s.dropped_foreign, s.short_frame, s.dropped_popcount
            ),
    ...
    dev.set_name_filter(false, false).expect("set_name_filter off");
    show(&dev, "t=0");
