...
pub const TX_INTENT: &[(&str, TxIntentSupport)] = &[
    ...
    ("Rtl8733buBackend", TxIntentSupport::Honoured("inject picks DESC_RATE_6M with flags cleared, instead of the stored tx_rate/tx_flags. ★ VERIFIED ON AIR 2026-09-01: witness read 376 frames @ 6.0 Mb/s on the MostRobust arm vs 65.0 Mb/s MCS7 on the Throughput arm, same run, stored rate MCS7")),
    ("Rtl8821cuBackend", TxIntentSupport::Honoured("build_tx forces DESC_RATE_OFDM6M where the DESC code is chosen, so a stored cur_mcs cannot bypass it. ⚠ NOT yet verified on air: the lab 8821cu enumerates in Realtek CDROM mode (0bda:1a2b) with no active configuration, so usb_modeswitch cannot open it — needs a physical replug")),
