...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("baseline: RF0x00={:05x} RF0x01={:05x} RF0x18={:05x}",
        dev.rf_read(0x00).unwrap_or(0), dev.rf_read(0x01).unwrap_or(0), dev.rf_read(0x18).unwrap_or(0));

    for v in [0x00u8, 0x1f, 0x0a, 0x1a] {
        dev.set_rf_txagc(v)?;
        let rb = dev.rf_read(0x01).unwrap_or(0);
        println!("  set_rf_txagc(0x{v:02x}) -> RF0x01={rb:05x} field[4:0]={:02x}  {}",
            rb & 0x1f,
            if (rb & 0x1f) as u8 == (v & 0x1f) { "MATCHES" } else { "*** MISMATCH — write did not land ***" });
    }
