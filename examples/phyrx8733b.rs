...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "{:>6} {:>8} {:>9} {:>9} {:>10} {:>7}",
        "cap", "frames", "snr_db", "evm_db", "cfo_hz", "rssi"
    );
    ...
    for d in [0i32, -40, 40, 0] {
        ...
        let (mut n, mut snr, mut evm, mut cfo, mut rssi, mut nphy) =
            (0usize, 0i64, 0i64, 0i64, 0i64, 0usize);
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(6);
        while n < per && std::time::Instant::now() < deadline {
            if let Ok(Ok(f)) =
                tokio::time::timeout(std::time::Duration::from_millis(200), dev.recv_frame()).await
            {
                n += 1;
                ...
                if let Some(p) = f.phy {
                    nphy += 1;
                    if let Some(v) = p.snr_db {
                        snr += i64::from(v);
                    }
                    if let Some(v) = p.evm_db {
                        evm += i64::from(v);
                    }
                    if let Some(v) = p.cfo_hz {
                        cfo += i64::from(v);
                    }
                }
            }
        ...
        if n == 0 {
            println!(
                "{cap:>6} {n:>8}   NO FRAMES RECEIVED — nothing to measure. Note ambient \
        ...
        if nphy == 0 {
            println!(
                "{cap:>6} {n:>8}   frames arrived but carried NO PHY metrics — check drvinfo \
    ...
    println!(
        "restored cap = {}  (last arm repeats the first: CFO should return, SNR/EVM never moved)",
        dev.crystal_cap()?
    );
    Ok(())
}
